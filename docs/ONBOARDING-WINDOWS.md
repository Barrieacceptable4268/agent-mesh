...
```bash
curl -fsSL https://raw.githubusercontent.com/moinsen-dev/agent-mesh/main/mesh -o ~/.local/bin/mesh
curl -fsSL https://raw.githubusercontent.com/moinsen-dev/agent-mesh/main/mesh-a2a.sh -o ~/.local/bin/mesh-a2a.sh
curl -fsSL https://raw.githubusercontent.com/moinsen-dev/agent-mesh/main/mesh-update.sh -o ~/.local/bin/mesh-update.sh
chmod +x ~/.local/bin/mesh ~/.local/bin/mesh-a2a.sh ~/.local/bin/mesh-update.sh
...
```bash
mesh init <hostname>     # z.B. "win-office" oder "win-laptop"
mesh sync                # Wissen exportieren + pushen
mesh role worker         # oder specialist
mesh agents              # prüfen: Agent ist sichtbar
```
...
```powershell
# PowerShell (als Admin): täglich 06:00 + 18:00 mesh sync
$action  = New-ScheduledTaskAction -Execute "C:\Program Files\Git\bin\bash.exe" `
  -Argument "-lc '/c/Users/$env:USERNAME/.local/bin/mesh sync'"
...
Der Webhook-Listener (`mesh-webhook.py`) läuft auf Windows als geplante
...
neue Nachrichten beim nächsten `mesh sync` (Task Scheduler 2× täglich).

## 7 · Update-Mechanismus (alle Plattformen)

```bash
mesh update --check     # Version prüfen (Lokal vs. Remote)
mesh update             # Framework pullen + installieren
mesh update --force     # Neu installieren trotz gleicher Version
```

Windows Task Scheduler (wöchentlich):
```powershell
$action  = New-ScheduledTaskAction -Execute "C:\Program Files\Git\bin\bash.exe" `
  -Argument "-lc '/c/Users/$env:USERNAME/.local/bin/mesh update'"
